...
// 1. Importación necesaria para el visualizador
...
export default function Page() {
  ...
  const deleteMedia = async (folder: "images" | "models", name: string) => {
    if (!confirm("¿Seguro que quieres borrar este contenido?")) return;
        ...
        {models.map((model) => (
          <Card key={model.name}>
            <div style={styles.modelWrapper}>
              {/* Usamos React.createElement para evitar errores de validación de etiquetas JSX */}
              {React.createElement("model-viewer", {
                src: model.url,
                alt: model.name,
                "auto-rotate": "", // En HTML nativo, los atributos booleanos pueden ir vacíos
                "camera-controls": "",
                "shadow-intensity": "1",
...
const styles: any = {
  ...
  modelWrapper: { width: "100%", height: 160, background: "#0f172a", borderRadius: 8, overflow: "hidden" },
